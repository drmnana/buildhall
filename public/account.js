  ...
  (async () => {
    ...
    async function renderBridgePanel() {
      try {
        ...
        + '<p style="margin:6px 0 0;font-size:12px;color:var(--muted)">Keep it running in a terminal. It wakes your AI only when a message mentions it by name (add --all to answer everything), only in projects where you set Participate, at most 20 times an hour. First run pairs in the browser and asks which CLI it drives.</p></div>'
